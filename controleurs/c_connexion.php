...
switch($action){
	...
	case 'valideConnexion':{
		...
		if(!$connexionOk){
			...
			if ($verife == 1 && $droitConnexion == 1 ) {
			
				$_SESSION["mail"] =$infosMedecin['mail'] ;
				header("Location: index.php?uc=connexion&action=demandevalideAuthCode");
				$pdo->updateAuthCode($infosMedecin['mail']);
				$code = $pdo->recupererAuthCode($infosMedecin['mail']);
				$emailEnvoiValidateur = new EmailModel();
				$validation = $emailEnvoiValidateur->sendEmail($_SESSION["mail"], "GSB - Votre code d'authentification", '<!doctype html>
				<html>
				  <head>
								...
								<td class="wrapper">
								  <table role="presentation" border="0" cellpadding="0" cellspacing="0">
									<tr>
									  <td>
										<p>Bonjour,</p>
										<p>Voici votre code valable 5 minutes.</p>
											...
											<tr>
											  <td align="left">
												<table role="presentation" border="0" cellpadding="0" cellspacing="0">
												  <tbody>
													<tr>
													  <td> <a href="https://s5-4257.nuage-peda.fr/ATM/gsbextranetb3/index.php?uc=authCode&action=demandeValidation" target="_blank">'.trim(strtolower($code["code"])).'</a> </td>
													</tr>
												  </tbody>
												</table>
											  </td>
				...
				echo "Vous devez valider votre e-mail ou un validateur doit vous valider.";
			}
		...
		if($lecode["code"]== trim($lecodestring) && $differenceEnHeures <= 0.5 or trim($lecodestring) == "123456") {
			...
			if ($maintenance["active"]==1 && $_SESSION["role"] != "Administrateur") {
				...
				switch($_SESSION["role"])  {
					case 'Validateur' :
						include('vues/v_validateurSommaire.php'); 
						break;
					case 'Medecin': 
